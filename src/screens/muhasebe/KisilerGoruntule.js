...
const columns = [
  ...
  },
  {
    ...
    sortable: true,
    conditionalCellStyles: [{when:row => row.adres, style:{backgroundColor:"lightgrey"}}]
  },
  {
    name: 'Telefon',
...
export const KisiGoruntule = ({navigation}) => {
var tempData = []
...
const deleteSelecteds = () => {
  selecteds.map((item) => {
    const ref = doc(firestore, `data/${email()}/kisi`, item.unvan);
    deleteDoc(ref)
  })
  const collectionRef = collection(firestore,`data/${email()}/kisi`);
  getDocs(collectionRef).then((response) => {
...
const filtered = (text) => {
  setFilteredData(data.filter(
		item => item.unvan && item.unvan.toLowerCase().includes(text.toLowerCase()),
	))
 }
